...
TSIResult calcTSI(
  ...
  final value = List<double>.generate(
    close.length,
    (i) => dsabs[i] != 0 ? 100 * dspc[i] / dsabs[i] : 0,
  );
...
RSIResult calcRSI(
  ...
  final maDown = List.generate(
    n,
    (i) => i > 0 ? rsiMA[i] <= rsiMA[i - 1] : true,
  );
...
BXtrenderResult calcBXtrender(
  ...
  final upDiff = rma(
    List.generate(n, (i) => diff[i] > 0 ? diff[i] : 0),
    shortL3,
  );
  final downDiff = rma(
    List.generate(n, (i) => diff[i] < 0 ? -diff[i] : 0),
  ...
  if (bxType == 'Short Term trend') {
    ...
    final upL = rma(
      List.generate(
        n,
        (i) =>
            i > 0 && close[i] - close[i - 1] > 0 ? close[i] - close[i - 1] : 0,
      ),
      longL2,
    );
    final downL = rma(
      List.generate(
        n,
        (i) =>
            i > 0 && close[i - 1] - close[i] > 0 ? close[i - 1] - close[i] : 0,
      ),
      longL2,
    );
...
ADXResult calcADX(
  ...
  final plusDM = rma(
    List.generate(n, (i) => up[i] > down[i] ? up[i] : 0),
    diLen,
  );
  final minusDM = rma(
    List.generate(n, (i) => down[i] > up[i] ? down[i] : 0),
    diLen,
  );
  final diPlus = fixnan(
  ...
  final adx = rma(
    List.generate(
      n,
      (i) => sum[i] != 0 ? 100 * (diPlus[i] - diMinus[i]).abs() / sum[i] : 0,
    ),
    adxLen,
  );
...
HACOLTResult calcHACOLT(
  ...
  final haCloseSmooth = List.generate(
    n,
    (i) => 2 * thaClose[i] - tema(thaClose, temaPeriod)[i],
  );
  final hl2Smooth = List.generate(
    n,
    (i) => 2 * thl2[i] - tema(thl2, temaPeriod)[i],
  );
...
AwesomeResult calcAwesome(
  List<double> high,
  List<double> low, {
  int fast = 5,
  int slow = 34,
  String type = 'Zero Line Cross',
}) {
  final n = high.length;
  final hl2 = List.generate(n, (i) => (high[i] + low[i]) / 2);
...
BBOSCResult calcBBOsc(
  ...
  final uPercent = List<double>.generate(
    n,
    (i) =>
        upper[i] != 0 ? (upper[i] - close[i]) / (upper[i] + close[i] / 2) : 0,
  );
  final lPercent = List<double>.generate(
    n,
    (i) =>
        lower[i] != 0 ? (lower[i] - close[i]) / (lower[i] + close[i] / 2) : 0,
  );
  final bPercent = List<double>.generate(
    n,
    (i) =>
        basis[i] != 0 ? (basis[i] - close[i]) / (basis[i] + close[i] / 2) : 0,
  );
